    ...
    return (
      <div className="bg-background py-12">
        <div className="container mx-auto px-8">
          <h2 className="text-h2 font-semibold text-neutral-900 mb-6">Albums</h2>
          <div className="bg-surface rounded-lg shadow-sm border border-neutral-100 overflow-hidden">
            <table className="w-full">
              <thead className="bg-neutral-50 border-b border-neutral-200">
                <tr>
                  <th className="text-left py-3 px-4 text-small font-semibold text-neutral-700">Cover</th>
                  <th className="text-left py-3 px-4 text-small font-semibold text-neutral-700">Album Name</th>
                  <th className="text-left py-3 px-4 text-small font-semibold text-neutral-700">Release Date</th>
                  <th className="text-left py-3 px-4 text-small font-semibold text-neutral-700">Tracks</th>
                  <th className="text-left py-3 px-4 text-small font-semibold text-neutral-700">Popularity</th>
                </tr>
              </thead>
              <tbody>
                {Array.from({ length: itemsPerPage }).map((_, index) => (
                  <tr key={index} className="animate-pulse">
                    <td className="py-3 px-4">
                      <div className="w-14 h-14 bg-neutral-200 rounded-sm"></div>
                    </td>
                    <td className="py-3 px-4">
              ...
              {albums.length === 0 ? (
                ...
                albums.map((album) => (
                  ...
                  >
                    <td className="py-3 px-4">
                      <img
                        src={album.urls && album.urls.length>0 ? album.urls[0] : '/api/placeholder/56/56'}
                        alt={album.name}
                    ...
                    <td className="py-3 px-4 text-body text-neutral-700">{album.num_tracks}</td>
                    <td className="py-3 px-4 text-body text-neutral-900 font-semibold">
                      {album.popularity}
                    </td>
                  </tr>
                ))
        ...
        {totalPages > 1 && (
          <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4 p-4 bg-surface rounded-lg border border-neutral-200">
            ...
            <div className="flex items-center gap-2">
              <label htmlFor="album-page-size" className="text-body-small text-neutral-600">
                Show:
              </label>
              <select
                id="album-page-size"
            ...
            <div className="text-body-small text-neutral-600">
              {totalItems > 0 
                ? `Showing ${startIndex}-${endIndex} of ${totalItems} albums`
